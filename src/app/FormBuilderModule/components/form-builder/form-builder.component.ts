...
export class FormBuilderComponent extends SnackBar implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.formElementsStyles$.pipe().subscribe(data => this.formElementsStylesSelector = data ? JSON.parse(data) : [])
    this.formStylesSelect$.pipe().subscribe(data => this.formStylesSelectorObs = data)
