...
export class DataChangingService {
  ...
  changeDataBeforeFieldsUpdate(data: object) {
    ...
    if (color) dataCopy['color'] = new Color(color.r, color.g, color.b, 1);
  ...
  changeFieldsDataBeforeSave(data: object) {
    let dataCopy = JSON.parse(JSON.stringify(data));
    if (dataCopy.width) dataCopy.width = dataCopy.width + 'px';
    if (dataCopy.height) dataCopy.height = dataCopy.height + 'px';
    if (dataCopy.fontSize) dataCopy.fontSize = dataCopy.fontSize + 'px';
    if (dataCopy.color.hex) dataCopy.color = '#' + dataCopy.color.hex;
